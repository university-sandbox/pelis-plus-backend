-- ================================================================
-- SEED: MONTH
-- Populates analytics data for the current calendar month.
-- Safe to re-run: deletes previous seed data before inserting.
-- Usage: psql "$DATABASE_URL" -f backend/scripts/seed-month.sql
-- ================================================================
...
DECLARE
  ...
  v_period_start_date DATE        := date_trunc('month', CURRENT_DATE)::DATE;
  v_days_in_period    INT         := CURRENT_DATE - date_trunc('month', CURRENT_DATE)::DATE;
  v_range_secs        BIGINT;
  ...
  -- Screening slot options
  v_slot_times   TIME[]    := ARRAY['10:00'::TIME,'13:30'::TIME,'16:00'::TIME,'19:00'::TIME,'22:00'::TIME];
  v_slot_formats TEXT[]    := ARRAY['standard','standard','3d','standard','imax'];
  v_slot_prices  NUMERIC[] := ARRAY[18.00, 18.00, 22.00, 18.00, 28.00];
...
BEGIN
  ...
  RAISE NOTICE '[seed-month] Cleanup complete.';
  ...
  RAISE NOTICE '[seed-month] 8 users created (4 with memberships).';
  ...
  FOR v_tmp_movie IN
    SELECT id, title FROM movies WHERE active = true ORDER BY popularity DESC NULLS LAST LIMIT 6
  LOOP
  ...
  IF array_length(v_movie_ids, 1) IS NULL THEN
    RAISE EXCEPTION '[seed-month] No active movies found — cannot seed.';
  END IF;
  ...
  IF array_length(v_room_ids, 1) IS NULL THEN
    RAISE EXCEPTION '[seed-month] No active rooms found — cannot seed.';
  END IF;
  ...
  RAISE NOTICE '[seed-month] Loaded: % movies, % rooms, % snacks.',
    array_length(v_movie_ids, 1), array_length(v_room_ids, 1), array_length(v_snack_ids, 1);

  -- ────────────────────────────────────────
  -- 6. SCREENINGS (15, spread across the month)
  -- ────────────────────────────────────────

  FOR i IN 0..14 LOOP
    v_movie_idx_pick := 1 + (i % array_length(v_movie_ids, 1));
    v_room_idx_pick  := 1 + (i % array_length(v_room_ids, 1));
    v_time_idx       := 1 + (i % 5);
    v_day_offset     := CASE
      WHEN v_days_in_period <= 0 THEN 0
      ELSE FLOOR(i::FLOAT * v_days_in_period / 14.0)::INT
    END;
    ...
    VALUES (
      ...
      v_period_start_date + v_day_offset,
      v_slot_times[v_time_idx],
      v_slot_formats[v_time_idx],
      v_slot_prices[v_time_idx],
      'available'
    );
    ...
    v_scr_dates     := v_scr_dates     || (v_period_start_date + v_day_offset);
    v_scr_times     := v_scr_times     || v_slot_times[v_time_idx];
    v_scr_formats   := v_scr_formats   || v_slot_formats[v_time_idx];
    v_scr_prices    := v_scr_prices    || v_slot_prices[v_time_idx];
  END LOOP;

  RAISE NOTICE '[seed-month] Created % screenings.', array_length(v_scr_ids, 1);
  ...
  RAISE NOTICE '[seed-month] Seats generated.';

  -- ────────────────────────────────────────
  -- 8. ORDERS (20, biased toward recent days)
  -- ────────────────────────────────────────

  v_range_secs := GREATEST(EXTRACT(EPOCH FROM (v_period_end - v_period_start))::BIGINT, 3600);
  ...
  FOR i IN 1..20 LOOP
    ...
    -- Timestamp biased toward recent days (sqrt pushes mass toward end of period)
    v_offset_secs := FLOOR(sqrt(random()) * v_range_secs)::BIGINT;
    v_created_at  := v_period_start + make_interval(secs => v_offset_secs::FLOAT);

    -- Random user
    v_user_id := v_user_ids[1 + (floor(random() * array_length(v_user_ids, 1)))::INT];

    -- Number of tickets (weighted: 65% → 1, 23% → 2, 12% → 3)
    v_num_tickets := CASE
  ...
  RAISE NOTICE '[seed-month] Done — 20 orders seeded across % day(s) of the month.', v_days_in_period + 1;
