-- ================================================================
-- SEED: WEEK
-- Populates analytics data for the last 7 days (rolling week).
-- Safe to re-run: deletes previous seed data before inserting.
-- Usage: psql "$DATABASE_URL" -f backend/scripts/seed-week.sql
-- ================================================================

DO $$
DECLARE
  -- Period: rolling 7 days
  v_period_start      TIMESTAMPTZ := date_trunc('day', NOW() AT TIME ZONE 'UTC') - INTERVAL '6 days';
  v_period_end        TIMESTAMPTZ := NOW();
  v_period_start_date DATE        := CURRENT_DATE - 6;
  v_range_secs        BIGINT;
...
BEGIN
  ...
  RAISE NOTICE '[seed-week] Cleanup complete.';
  ...
  RAISE NOTICE '[seed-week] 8 users created (4 with memberships).';
  ...
  IF array_length(v_movie_ids, 1) IS NULL THEN
    RAISE EXCEPTION '[seed-week] No active movies found — cannot seed.';
  END IF;
  ...
  IF array_length(v_room_ids, 1) IS NULL THEN
    RAISE EXCEPTION '[seed-week] No active rooms found — cannot seed.';
  END IF;
  ...
  RAISE NOTICE '[seed-week] Loaded: % movies, % rooms, % snacks.',
    array_length(v_movie_ids, 1), array_length(v_room_ids, 1), array_length(v_snack_ids, 1);

  -- ────────────────────────────────────────
  -- 6. SCREENINGS (10, one or two per day across 7 days)
  -- ────────────────────────────────────────
  -- Day offsets: 0,0,1,2,2,3,4,4,5,6 → distributes 10 screenings across days 0-6

  FOR i IN 0..9 LOOP
    v_movie_idx_pick := 1 + (i % array_length(v_movie_ids, 1));
    v_room_idx_pick  := 1 + (i % array_length(v_room_ids, 1));
    v_time_idx       := 1 + (i % 5);
    -- Evenly distribute across 6 day range (CURRENT_DATE-6 to today)
    v_day_offset     := FLOOR(i::FLOAT * 6 / 9.0)::INT;
  ...
  RAISE NOTICE '[seed-week] Created % screenings.', array_length(v_scr_ids, 1);
  ...
  RAISE NOTICE '[seed-week] Seats generated.';

  -- ────────────────────────────────────────
  -- 8. ORDERS (18, uniformly spread across 7 days)
  -- ────────────────────────────────────────
  ...
  FOR i IN 1..18 LOOP
    ...
    -- Uniform distribution across the week
  ...
  RAISE NOTICE '[seed-week] Done — 18 orders seeded across the last 7 days.';
